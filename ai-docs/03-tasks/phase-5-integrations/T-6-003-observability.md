...
- В реальный runtime (`project_config`) встроен `django-prometheus`, а не в legacy `Config/*`.
- Добавлены custom metrics для notification delivery и active SSE connections.
- `docker-compose.yml` дополнен `prometheus` и `grafana`, добавлены provisioning-конфиги и sample alert rules.
- Написаны `ai-docs/06-integrations/observability-runbook.md` и `ai-docs/08-reports/T-6-003.md`.

### Что остаётся владельцу

- Установить новые зависимости в серверный env и поднять observability stack после `T-6-001`.
- Настроить внешний uptime monitor на `/api/v1/health/live`.
- Настроить реальную доставку alert'ов владельцу (Grafana alerting / Alertmanager).
