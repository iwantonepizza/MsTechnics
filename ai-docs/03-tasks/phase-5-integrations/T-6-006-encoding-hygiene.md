...
Архитектор реально видел в `Read`-выводе строки вида `# 03-tasks вЂ" Р·Р°РґР°С‡Рё РґР»СЏ РєРѕРґРµСЂРѕРІ` вместо `# 03-tasks — задачи для кодеров`. Поправил через `Write` в правильной кодировке.
...
3. **Pre-commit hook** — простой Python/bash скрипт, который проверяет, что все `*.md`, `*.py`, `*.txt` в staged changes:
   ...
   - не содержат типичные mojibake-паттерны (например, `Р·Р°РґР°С‡` — кириллица, перекодированная через cp1251).
...
MOJIBAKE_MARKERS = [
    ...
    "вЂ”", "вЂ"", "вЂ"", "вЂ¦",                       # mangled em-dash / quotes
    "РЎС‚Р°С‚СѓСЃ",                                     # mangled "Статус"
]
