...
GET    /api/v1/me                 → 200 { id, username, email, permission, roles: string[],
                                         extra_permissions: string[], allowed_cities: [...],
                                         telegram_id, max_chat_id, show_activity_feed }
...
```
GET    /api/v1/displays?city=<slug>                      → [{ id, name, description, slug, city, rows, cols, current_condition }]
...
```json
{
  ...
  "project_photo_url": "/media/projects/izh-1.jpg",
  "cells": [
    {
      "id": 101,
      "position": "01",
...
DELETE /api/v1/panels/<id>                                              → 204 (только admin)
...
`DELETE /panels/<id>` запрещён при активной заявке (`409 panel_has_active_application`). Для
архивной панели за одну транзакцию удаляются связанные архивные заявки, их события, ActivityLog и
legacy history; после удаления создаётся обезличенное событие `panel.deleted`.
...
```
GET    /api/v1/activity-log?display=<slug>&panel=<id>&cell=<id>&kind=<prefix>&event_types=<csv>&since=<...>&before=<...>
       → cursor-paginated [{ id, event_type, target_kind, target_id, user, timestamp, comment, file, payload }]

       kind — prefix `event_type`, например `application`, `panel`, `display`
       event_types — точный CSV-набор, например `panel_move,panel.removed`
```

Единый endpoint для всех историй (задача владельца #11).
Общая лента без `display`/`panel`/`cell` доступна admin или пользователю с
`show_activity_feed=true`; для обычного пользователя backend ограничивает её разрешёнными городами.

### 8.1. Ежедневные задачи

```
GET  /api/v1/daily-tasks?city=<id>              → cursor-paginated DailyTask[]
GET  /api/v1/daily-tasks/<id>                   → DailyTask
POST /api/v1/daily-tasks/<id>/complete          → 200 DailyTask
```

- мониторинг видит и выполняет доступные задачи;
- контроль видит тот же список read-only;
- admin видит все города;
- `city` не может обойти `allowed_city`; нечисловой `city` возвращает 422.
