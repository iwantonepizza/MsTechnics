...
class DashboardView(APIView):
    ...
    def get(self, request):
        user = request.user
        is_admin = user.permission in ("admin", "all")
        from apps.workflow.applications.models import Application
        qs = Application.objects.select_related("status")
        if not is_admin:
        ...
        def recent(status_names, limit=5):
            ...
            apps = qs.filter(status__name__in=status_names).order_by("-last_update_date_time")[:limit]
