...
@extend_schema_view(
    list=extend_schema(tags=["displays"], summary="Список экранов",
...
class DisplayViewSet(ReadOnlyModelViewSet):
    ...
    def photos(self, request, slug=None):
        ...
        return Response([
            {"id": p.id, "url": p.image.url if p.image else None,
             "uploaded_at": getattr(p, "uploaded_at", None)}
            for p in photos
        ])

    @extend_schema(tags=["displays"], summary="Загрузить фото",
                   request={"multipart/form-data": PhotoUploadSerializer})
    @action(detail=True, methods=["post"], url_path="photos/upload",
    ...
    def upload_photo(self, request, slug=None):
        ...
        return Response({"id": photo.id, "url": photo.image.url}, status=http_status.HTTP_201_CREATED)
