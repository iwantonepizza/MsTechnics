...
def edit_panel_comment(request):
    logger.info("command edit_panel_comment start")
    if request.method == 'POST':
        form = EditPanelComment(request.POST)
        if form.is_valid():
            panel_id = form.cleaned_data['pannel_id']
            new_comment = form.cleaned_data['new_comment']
            try:
                panel = Panels.objects.get(name=panel_id)
                panel.comment = new_comment
                panel.save()
                return HttpResponse("OK")
            except Panels.DoesNotExist:
                return HttpResponseBadRequest("Panel not found")
